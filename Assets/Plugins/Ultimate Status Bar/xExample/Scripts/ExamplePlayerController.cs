...
public class ExamplePlayerController : MonoBehaviour
{
	...
	void Update ()
	{
		...
		if( Input.GetKeyDown( KeyCode.Alpha1 ) && skillInCooldown == false )
			StartCoroutine( "ThunderStrikeCooldown" );
	}

	IEnumerator ThunderStrikeCooldown ()
	{
		skillInCooldown = true;
		for( float t = 1.0f; t > 0.0f; t -= Time.deltaTime * 0.5f )
		{
			UltimateStatusBar.UpdateStatus( "ThunderStrike", t, 1.0f );
			yield return null;
		}
		UltimateStatusBar.UpdateStatus( "ThunderStrike", 0.0f, 1.0f );
